...
def count_words(subreddit, word_list, after=None, word_count={}):
    ...
    for title in titles:
        words = title.lower().split()  # Split the title into words, case-insensitive
        for word in word_list:
            word_count[word] = word_count.get(word, 0) + words.count(word)

    # Recursion if there's a next page (after token)
    ...
    if after is None:
        ...
        filtered_word_count = {word: count for word, count in word_count.items() if count > 0}

        # Sort by count (descending), then by word (alphabetically)
        sorted_word_count = sorted(filtered_word_count.items(), key=lambda item: (-item[1], item[0]))
...
word_list = ['python', 'javascript', 'java', 'C++', 'ruby', 'python']
